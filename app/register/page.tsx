...
import { useRouter } from "next/navigation"; // NOVO: para redirecionar após registro
...
export default function RegisterPage() {
  ...
  // NOVO: função que registra o usuário no Supabase Auth e salva nome/CEP na tabela perfis
  async function handleRegistrar() {
    ...
    await supabase.from("perfis").insert({ id: data.user!.id, nome, cep });
    setCarregando(false);
    alert("Conta criada com sucesso!");
    router.push("/login"); // NOVO: redireciona para o login após registro
  }
          ...
          <div className="flex flex-col gap-7 w-full">
            ...
            <div className="flex flex-col gap-4">
              <input
                ...
                value={nome}
                onChange={(e) => { setNome(e.target.value); if (e.target.value.trim()) setMostrarEmail(true); }}
                className={inputBase}
              />
              ...
              <div className={fade(mostrarEmail)}>
                <input
                  ...
                  value={email}
                  onChange={(e) => { setEmail(e.target.value); if (e.target.value.trim()) setMostrarSenha(true); }}
                  className={inputBase}
                />
              </div>
              ...
              <div className={fade(mostrarSenha)}>
                <input
                  ...
                  value={senha}
                  onChange={(e) => { setSenha(e.target.value); if (e.target.value.trim()) setMostrarCep(true); }}
                  className={inputBase}
                />
              </div>
              ...
              <div className={fade(mostrarCep)}>
                <input
                  ...
                  value={cep}
                  onChange={(e) => { setCep(e.target.value); if (e.target.value.trim()) setMostrarTermos(true); }}
                  className={inputBase}
                />
              </div>
              ...
              {/* ALTERADO: adicionado onClick, disabled e texto de loading no botão */}
 ...
 *            Adicionado para redirecionar o usuário após o registro.
 *
 * Linha 10 — import { supabase } from "@/lib/supabase"
 *            Adicionado o cliente Supabase criado em lib/supabase.ts.
 *
 * Linha 20 — const router = useRouter()
 *            Hook de navegação inicializado dentro do componente.
 *
 * Linha 30 — const [carregando, setCarregando] = useState(false)
 *            Novo estado para desabilitar o botão enquanto registra.
 *
 * Linhas 38-54 — async function handleRegistrar()
 *            Função nova que:
 *            - Valida se todos os campos estão preenchidos
 *            - Chama supabase.auth.signUp() com email e senha
 *            - Insere nome e CEP na tabela "perfis"
 *            - Redireciona para /login após sucesso
 *
 * Linhas 115-120 — botão REGISTRAR
 *            Adicionado onClick={handleRegistrar}, disabled={carregando}
 *            e texto dinâmico "REGISTRANDO..." durante o loading.
 */